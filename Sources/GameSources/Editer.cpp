		...
		if (::GetAsyncKeyState(VK_MENU) == 0)
		{
			for (size_t i =0; i<EnemyManager::GetInstance()->GetEnemyList().size();i++)
			{
				if (::GetAsyncKeyState(VK_LBUTTON) & 0x8000)
				{
					...
					{
						if (collision.SphereVsSphere(DirectX::XMFLOAT3(screenWorld.x,0, screenWorld.z),
							DirectX::XMFLOAT3(EnemyManager::GetInstance()->GetEnemyList().at(i)->GetPos().x, 0, EnemyManager::GetInstance()->GetEnemyList().at(i)->GetPos().z),100,100))
						{
