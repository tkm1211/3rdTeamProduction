...
// ノード推論
NodeBase *NodeBase::Inference(AI *obj, BehaviorData *data)
{
	...
	{
		if (m_Child[i]->m_ExecJudgement != NULL)
		{
			if (m_Child[i]->m_ExecJudgement->Judgement(obj) == true)
			{
...
NodeBase *NodeBase::SelectSequence(std::vector<NodeBase*> *list, BehaviorData *data)
{
	...
	// シーケンスが終わってたら終了
	if (step >= static_cast<int>(m_Child.size()))
	{
...
// 判定
bool NodeBase::Judgement(AI *obj)
{
	//　判定クラスを持っていれば実効なければtrue扱い
	if (m_ExecJudgement != NULL)
	{
		return m_ExecJudgement->Judgement(obj);
	}
	return true;
}

// ノード実行
ActionBase::STATE NodeBase::Update(AI *obj)
{
	// アクションノードがあれば実行して結果を返す、なければ失敗扱い
	if (m_Action != NULL)
	{
		return m_Action->Update(obj);
	}
